...
# Bright Colors (already defined, but explicitly listed for clarity)
...
# Background Colors (already defined, but explicitly listed for clarity)
BG_BLACK = "\033[40m"
BG_RED = "\033[41m"
BG_GREEN = "\033[42m"
BG_YELLOW = "\033[43m"
...
# Bright Background Colors (already defined, but explicitly listed for clarity)
...
def display_debit_cards():
    ...
    print(f"\n{YELLOW}{'=' * 50}{RESET}") # Yellow border
    # Changed from \033[34m to BLUE for consistency
    print(f"{BOLD}{BLUE}Debit Cards{RESET}".center(50 + len(BOLD) + len(BLUE) + len(RESET))) # Centered, bold blue
    # Changed from \033[33m to YELLOW for consistency
    print(f"{YELLOW}{'=' * 50}{RESET}") # Yellow border
    # Changed from \033[32m to GREEN for consistency
    print(f"{GREEN}1. Club Debit MasterCard{RESET}") # Green text
    print(f"{GREEN}2. Debit Visa{RESET}") # Green text
    print(f"{GREEN}3. Gold MasterCard{RESET}") # Green text
    # Changed from \033[33m to YELLOW for consistency
    print(f"{YELLOW}P. Go back to previous menu{RESET}") # Yellow text
    print(f"{YELLOW}M. Go to main menu{RESET}") # Yellow text
    # Changed from \033[33m to YELLOW for consistency
...
def display_prepaid_cards():
    ...
    print(f"\n{YELLOW}{'=' * 50}{RESET}") # Yellow border
    # Changed from \033[34m to BLUE for consistency
    print(f"{BOLD}{BLUE}Prepaid Cards{RESET}".center(50 + len(BOLD) + len(BLUE) + len(RESET))) # Centered, bold blue
    # Changed from \033[33m to YELLOW for consistency
    print(f"{YELLOW}{'=' * 50}{RESET}") # Yellow border
    # Changed from \033[32m to GREEN for consistency
    print(f"{GREEN}1. Multi Currency Prepaid MasterCard{RESET}") # Green text
    print(f"{GREEN}2. Sapphire Prepaid Visa{RESET}") # Green text
    print(f"{GREEN}3. Safari Prepaid Visa{RESET}") # Green text
    # Changed from \033[33m to YELLOW for consistency
    print(f"{YELLOW}P. Go back to previous menu{RESET}") # Yellow text
    print(f"{YELLOW}M. Go to main menu{RESET}") # Yellow text
    # Changed from \033[33m to YELLOW for consistency
...
def display_credit_cards():
    ...
    print(f"\n{YELLOW}{'=' * 50}{RESET}") # Yellow border
    # Changed from \033[34m to BLUE for consistency
    print(f"{BOLD}{BLUE}Credit Cards{RESET}".center(50 + len(BOLD) + len(BLUE) + len(RESET))) # Centered, bold blue
    # Changed from \033[33m to YELLOW for consistency
    print(f"{YELLOW}{'=' * 50}{RESET}") # Yellow border
    # Changed from \033[32m to GREEN for consistency
    print(f"{GREEN}1. Gold Visa Credit Card{RESET}") # Green text
    print(f"{GREEN}2. Bronze Credit MasterCard{RESET}") # Green text
    print(f"{GREEN}3. Diamond Credit Card{RESET}") # Green text
    # Changed from \033[33m to YELLOW for consistency
    print(f"{YELLOW}P. Go back to previous menu{RESET}") # Yellow text
    print(f"{YELLOW}M. Go to main menu{RESET}") # Yellow text
    # Changed from \033[33m to YELLOW for consistency
...
def display_card_details_info(card_type, specific_card=None):
    ...
    if card_type == 1: # Debit Cards
        ...
        # Use RED_X for error
        print(f"{RED_X} Invalid card type.{RESET}")
        return None
...
def handle_account_services_flow(current_username):
    ...
    while True:
        ...
        if service_choice == 1: # View Account Details
            ...
            print(f"{BLUE_INFO} Card management features are under development.{RESET}")
            input(f"{BRIGHT_BLACK}Press Enter to continue...{RESET}")
        ...
        if current_username is None: # Not logged in
            if choice == 1: # Open a bank account
                ...
                if username in accounts_data and accounts_data[username]["password"] == password:
                    ...
                    available_balance_padding = balance_line_len - len(strip_ansi_codes(available_balance_label)) - len(available_balance_value)
                    print(f"{available_balance_label}{' ' * available_balance_padding}{available_balance_value}")

                    # Current Balance line (for simplicity, same as available)
                    current_balance_label = f"{BLUE}Current Balance:{RESET}"
                    current_balance_value = f"{current_balance:,.2f} {account_currency}"
                    current_balance_padding = balance_line_len - len(strip_ansi_codes(current_balance_label)) - len(current_balance_value)
                    print(f"{current_balance_label}{' ' * current_balance_padding}{current_balance_value}")
                ...
                print(f"{BLUE_INFO}Thank you for using the Python Bank Simulation. Goodbye!{RESET}")
                break
            else:
                print(f"{RED_X} Invalid choice. Please enter 1, 2, 3, or 4.{RESET}")
                input(f"{BRIGHT_BLACK}Press Enter to continue...{RESET}")
        else: # Logged in
            if choice == 1: # Account Services
                ...
                print(f"\n{BLUE_INFO}Logging out {current_username}. Returning to main menu.{RESET}")
